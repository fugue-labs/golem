...
Golem is a self-built (see `/mission`), fully autonomous, long-running coding agent without approvals or HITL. **Exercise caution**. It has been building itself for a few days now. I have written zero LoC of golem, personally. The current state of the codebase was generated by running a few missions. I built this to flex the underlying agent framework, [gollem](https://github.com/fugue-labs/gollem), however _golem_ (this repo) is proving value, and has become my default coding agent CLI / TUI of choice.
